...
void GameClass::exportFileHeaders(GameFileClass& files)
{
	files.fileHeader.startTime = startTime;
	files.fileHeader.viewercursor_x = viewerCursor.x;
	files.fileHeader.viewercursor_y = viewerCursor.y;
	files.fileHeader.gamemode = player.gamemode;
	files.fileHeader.inv_cellcount = int(player.inv.cellList.size());
	for (int i = 0; i<int(player.inv.cellList.size()); i++)
	{
		saveFileInventory dummy;
		dummy.inv_itemcount = player.inv.getQuantityAt(i);
		dummy.template_idx = player.inv.cellList[i].tmp_idx;
		for (int t = 0; t < dummy.inv_itemcount; t++)
		{
			saveFileItems cell;
			cell.ether_index = player.inv.cellList[i].l.items[t];
			dummy.items.push_back(cell);
		}
		files.fileHeader.inventoryCells.push_back(dummy);
	}
}
